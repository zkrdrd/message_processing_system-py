...
class Environment:
    ...
    def get_env_storage(self) -> str:
        """Чтение полей 'ENV_STORAGE_TYPE' и
        'ENV_STORAGE_FILE_PATH' из environment"""
        return os.environ.get("ENV_STORAGE_TYPE"), os.environ.get(
            "ENV_STORAGE_FILE_PATH"
        )

    def use_storage(
        self,
        env_storage_type: str,
        env_storage_file_path: str,
    ) -> object:
                ...
                logger.warning(
                    'storage type is not found.\
                        Using default storage in memory.\
                            For switch storage use "ENV_STORAGE_TYPE"'
                )
                storage = StorageInMemory()
        return storage
