...
class StorageInSQLite():
	...
	ON CONFLICT DO UPDATE SET 
        ...
        self.__save_payment_fields = (payment.type_message, payment.uid_message,	payment.address_from, payment.address_to, 
                            payment.amount, payment.created_at, payment.updated_at)
    ...
    def get_payment_by_id(self, id:str) -> Payment:
        ...
        self.__query_select = '''SELECT type_message, uid_message, address_from, address_to, amount, created_at, updated_at 
	FROM payment WHERE uid_message = ?;'''
        ...
        try:
            with sqlite3.connect(self.__sqlite_storage) as self.__connect:
                ...
                if self.__columns == None or self.__values == None:
                    return None
                else:
